...
class BaseKineticMC:
    def __init__(self, temp_C=250, reaction_time=7200, chain_length=None,
        ...
        default_params = {
            # 1. Adsorption/Desorption Group
            'A_ads':     1e6,    # Pre-exponential for adsorption
            'E0_ads':    0.2,    # Base barrier (eV)
            'A_d':       1e13,   # Pre-exponential for desorption
            'E0_d':      0.8,    # Base barrier (eV)
            'alpha_vdw': 0.05,   # vdW chain-length scaling (eV per carbon)

            # 2. Reaction Group (dMC & Scission)
            'A_base':   1e12,
            'E_dMC':    1.1,     # dMC formation barrier (eV)
            'E_crk':    1.4,     # C-C scission barrier (eV)
            'beta_int': 0.1,     # Internal position penalty (eV)

            # 3. Hydrogen Equilibrium
            'K_H2': 0.01,        # Langmuir equilibrium constant (bar^-1)
        }
    ...
    def normal_dist(self, mu=260, sigma=30, n_samples=None):
        ...
        if n_samples is None:
            return int(np.floor(np.random.normal(loc=mu, scale=sigma)))
    ...
    def get_rate(self, N, reaction_type, is_internal=False):
        ...
        if reaction_type == 'adsorption':
            barrier = p['E0_ads'] - p['alpha_vdw'] * N
            return p['A_ads'] * np.exp(-barrier / kT)

        elif reaction_type == 'desorption':
            barrier = p['E0_d'] + p['alpha_vdw'] * N
            return p['A_d'] * np.exp(-barrier / kT)

        elif reaction_type == 'dMC':
            barrier = p['E_dMC'] + p['beta_int'] * is_internal
            return p['A_base'] * np.exp(-barrier / kT)

        elif reaction_type == 'cracking':
            barrier = p['E_crk'] + p['beta_int'] * is_internal
            return p['A_base'] * np.exp(-barrier / kT)
    ...
    def init_arrays(self, chain_length):
        ...
        carbon_to_site[j] : surface C site index holding carbon j (-1 if free)
        """
    ...
    def init_active_sites(self, catalyst_config=None):
        ...
        C-site arrays  (length = n_c_sites):
            ...
            chain_at_site[i]  : fragment length of carbon at site i (0 if vacant)
        ...
        """
        from CatConfig import CatalystSurface, pt111_config
